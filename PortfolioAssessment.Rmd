...
## Our portfolio as of `r Sys.Date()`
```{r echo=FALSE, message = FALSE, warning = FALSE}
date <- gsub("-", "", today())
...
The Value of the current portfolio is `r sum(Portfolio.df$CP)`, allocated as follows.
```{r echo=FALSE, message = FALSE, warning = FALSE}
...
Summary[ , c("Ticker", "AP", "CP", "weight", "StopLossFinal", "Signal", "Quality", "Momentum", "Final_rank")] %>%
  mutate(weight = round(weight, 4),
         StopLossFinal = round(weight, 2)) %>% 
